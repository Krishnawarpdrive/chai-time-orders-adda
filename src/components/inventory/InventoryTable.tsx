...
import { Search, ChevronDown, ChevronUp, AlertTriangle, Bell, RefreshCw } from 'lucide-react';
...
const InventoryTable = () => {
  const { inventory, loading, error, updateInventoryItem, refreshInventory } = useInventory();
  ...
  useEffect(() => {
    console.log('Inventory data changed:', inventory);
    ...
    return (
      <div className="bg-red-50 border border-red-200 rounded-md p-4 mb-4">
        <div className="flex items-center gap-2 text-red-700 mb-2">
          ...
          <p className="text-sm font-medium">Error loading inventory</p>
        </div>
      ...
      {inventory.length === 0 ? (
        <div className="text-center py-12">
          ...
          <p className="mt-1 text-sm text-gray-500">
            Your inventory appears to be empty. Add some items to get started.
          </p>
        </div>
      ) : (
